...
def Levenshtein(a, len_a, b, len_b):
    if len_a == 0 or len_b == 0:
        return len_a if (len_b == 0) else len_b

    re1 = Levenshtein(a, len_a - 1, b, len_b) + 1
    re2 = Levenshtein(a, len_a, b, len_b - 1) + 1
    re3 = Levenshtein(a, len_a - 1, b, len_b - 1) + (0 if (a[len_a - 1] == b[len_b - 1]) else 1)
...
def similarity_measure(src_1, src_2):
    ...
    NodeVector(ast_root_1)
    NodeVector(ast_root_2)

    CalculateVector(ast_root_1)
    CalculateVector(ast_root_2)
    ...
    cos = dot(vec1, vec2) / (distance(vec1, [0 for i in vec1]) * distance(vec2, [0 for i in vec1]))
    ...
    Levenshtein_similarity = 1 - Levenshtein_distance(str1, str2) / maxLen
...
print(similarity_measure("./src_code_1.py", "./src_code_2.py"))
