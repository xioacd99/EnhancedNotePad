...
class AES(object):  # AES-128-CBC
    def __init__(self):
        self.state = [[0 for j in range(4)] for i in range(4)]  # 4 x 4, 16 * 8 = 128 bits
        self.sbox = genSBox()
        self.revsbox = genRevSBox()
        self.roundKey = []
    ...
    def invMixColumn(self):
        ...
        curr = self.state
        for i in range(4):
            tmp[0][i] = polynomialMutil(curr[0][i], 14) ^ polynomialMutil(curr[1][i], 11) ^ polynomialMutil(curr[2][i],
                                                                                                            13) ^ polynomialMutil(
                curr[3][i], 9)
            tmp[1][i] = polynomialMutil(curr[0][i], 9) ^ polynomialMutil(curr[1][i], 14) ^ polynomialMutil(curr[2][i],
                                                                                                           11) ^ polynomialMutil(
                curr[3][i], 13)
            tmp[2][i] = polynomialMutil(curr[0][i], 13) ^ polynomialMutil(curr[1][i], 9) ^ polynomialMutil(curr[2][i],
                                                                                                           14) ^ polynomialMutil(
                curr[3][i], 11)
            tmp[3][i] = polynomialMutil(curr[0][i], 11) ^ polynomialMutil(curr[1][i], 13) ^ polynomialMutil(curr[2][i],
                                                                                                            9) ^ polynomialMutil(
                curr[3][i], 14)
        self.state = tmp
