...
from PyQt5.QtWidgets import QMainWindow, QGridLayout, QWidget, QPushButton, QFileDialog, QTextEdit, QLabel
from wordcloud import WordCloud


class WordCloudPic(QMainWindow):
    ...
    def initUI(self):
        ...
        self.button1 = QPushButton(qtawesome.icon('fa.sellsy', color='white'), "英文文本统计图")  # 创建按钮1
        self.left_layout.addWidget(self.button1, 2, 0, 1, 3)
        self.button1.clicked.connect(self.generate_charts)
        self.button2 = QPushButton(qtawesome.icon('fa.sellsy', color='white'), "生成英文词云图")  # 创建按钮2
        self.left_layout.addWidget(self.button2, 7, 0, 1, 3)
        self.button2.clicked.connect(self.save_code)
        self.button3 = QPushButton(qtawesome.icon('fa.sellsy', color='white'), "生成中文词云图")  # 创建按钮3
    ...
    def generate_wordcloud(self):
        try:
            ...
            self.right_layout.addWidget(self.label_image, 0, 0, 4, 8)
            filename = QFileDialog.getOpenFileName(self, 'Open file', '.')[0]
            wordcloud_pic(filename)

            self.image = QtGui.QPixmap('wordcloud.png').scaled(self.label_image.width(), self.label_image.height())
